...
public partial class pages_Form_FeedbackMaster_Admin : System.Web.UI.Page
{
    ...
    private void fnLoadData(Boolean DoRebind)
    {
        try
        {
            ...
            string query = "SELECT tbl_User_Feedback.Ticket_Id, tbl_User_Feedback.Feedback, tbl_User_Feedback.Created_Time, tbl_Type_Master.Type_Name,tbl_User_Master.User_First_Name + ' ' + tbl_User_Master.User_Last_Name AS userName, tbl_Application_Master.Application_Name, tbl_Issue_Master.Issue_Name FROM         tbl_Type_Master INNER JOIN tbl_Ticket_Master ON tbl_Type_Master.Type_Id = tbl_Ticket_Master.Type_Id INNER JOIN tbl_User_Feedback ON tbl_Ticket_Master.Ticket_Id = tbl_User_Feedback.Ticket_Id INNER JOIN tbl_User_Master ON tbl_Ticket_Master.Created_By = tbl_User_Master.User_Id AND tbl_Ticket_Master.Type_Id IN (SELECT     Type_Id FROM fnAdminAccess() WHERE      (User_Email = '" + userEmail + "')) INNER JOIN tbl_Application_Master ON tbl_Ticket_Master.Application_Id = tbl_Application_Master.Application_Id INNER JOIN tbl_Issue_Master ON tbl_Ticket_Master.Issue_Id = tbl_Issue_Master.Issue_Id ORDER BY tbl_User_Feedback.Created_Time DESC";
